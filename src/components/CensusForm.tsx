...
export const CensusForm: React.FC<CensusFormProps> = ({ onResetToHero }) => {
  ...
  const StepperBar = () => (
    <div className="mb-6 glass-card rounded-2xl p-4 border border-slate-800/80">
      {/* Step pills row */}
          ...
          return (
            <React.Fragment key={num}>
              <div className="flex flex-col items-center gap-1 min-w-0">
                <div
                  className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center font-bold text-sm transition-all
                    ${active  ? 'bg-gradient-to-br from-brand-500 to-amber-500 text-white shadow-lg shadow-brand-500/30 scale-110'  : ''}
                    ${done    ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' : ''}
                    ${future  ? 'bg-slate-800 text-slate-500' : ''}
                  `}
                >
                  {done ? '✓' : <Icon className="w-4 h-4 sm:w-5 sm:h-5" />}
                </div>
                <span className={`hidden sm:block text-[10px] font-semibold leading-none text-center
      ...
      {/* Mobile step label */}
      <p className="sm:hidden mt-2 text-center text-xs font-semibold text-amber-400 tracking-wide">
    ...
    <div className="w-full max-w-2xl mx-auto px-0">
      ...
      {step < 4 && (
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-white mb-4 transition group"
        >
      ...
      {/* Step panels */}
      {step < 4 && (
        <div className="glass-card rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-slate-700/60 shadow-2xl">
          {step === 1 && (
            <StepPersonalData
              formData={formData}
